...
export default function LandingPage() {
  return (
    <div style={{ minHeight: '100vh', background: '#04060f', color: '#f1f5ff', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>

      {/* ── Navbar ── */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 48px', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, background: 'rgba(4,6,15,0.92)', backdropFilter: 'blur(12px)', zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 8, background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 900 }}>N</div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <Link to="/login" style={{ padding: '8px 20px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', color: '#94a3b8', textDecoration: 'none', fontSize: 14, fontWeight: 500, transition: 'all .2s' }}
      ...
      <section style={{ textAlign: 'center', padding: '100px 24px 80px', position: 'relative', overflow: 'hidden' }}>
        {/* Glow background */}
        <div style={{ position: 'absolute', top: '10%', left: '50%', transform: 'translateX(-50%)', width: 600, height: 600, background: 'radial-gradient(circle, rgba(99,102,241,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
        ...
        <h1 style={{ fontSize: 'clamp(36px, 6vw, 72px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 24, background: 'linear-gradient(135deg, #f1f5ff 0%, #a5b4fc 50%, #8b5cf6 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
        ...
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link to="/login" style={{ padding: '14px 36px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.12)', color: '#f1f5ff', textDecoration: 'none', fontSize: 15, fontWeight: 600, background: 'rgba(255,255,255,0.03)' }}>
            Se connecter
          </Link>
        </div>
      </section>
      ...
      {/* ── Stats ── */}
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.03)' }}>
        {STATS.map((s, i) => (
      ...
      <section style={{ padding: '80px 48px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <h2 style={{ fontSize: 36, fontWeight: 800, marginBottom: 12 }}>Tous vos métiers,<span style={{ color: '#8b5cf6' }}> une seule plateforme</span></h2>
          <p style={{ color: '#64748b', fontSize: 15 }}>Chaque module est activable selon le profil de votre entreprise</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
          {MODULES.map((m, i) => (
            <div key={i} style={{ padding: '24px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)', transition: 'border-color .2s, background .2s', cursor: 'default' }}
              ...
              <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6, color: '#f1f5ff' }}>{m.label}</div>
      ...
      {/* ── Features ── */}
      <section style={{ padding: '60px 48px', background: 'rgba(99,102,241,0.04)', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 40 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6, color: '#f1f5ff' }}>{f.title}</div>
      ...
      <section style={{ textAlign: 'center', padding: '80px 24px' }}>
        <h2 style={{ fontSize: 36, fontWeight: 800, marginBottom: 16 }}>Prêt à digitaliser votre entreprise ?</h2>
        <p style={{ color: '#64748b', fontSize: 15, marginBottom: 36 }}>Créez votre compte gratuitement et accédez à tous les modules en quelques minutes.</p>
      ...
      {/* ── Footer ── */}
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '24px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#334155', fontSize: 13 }}>
        <span style={{ fontWeight: 700, color: '#475569' }}>NexusERP</span>
